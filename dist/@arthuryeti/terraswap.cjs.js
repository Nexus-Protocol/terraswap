...
var terra = require('@arthuryeti/terra');
...
var minAmountReceive = function minAmountReceive(_ref) {
  ...
  var rate1 = terra.num("1").minus(maxSpread);
  return terra.num(amount).times(rate1).toString();
};
var priceImpact = function priceImpact(_ref2) {
  var offerAmount = _ref2.offerAmount,
      maxSpread = _ref2.maxSpread;
  var amount = terra.num(offerAmount);
  var spread = terra.num(maxSpread);
...
var createSwapMsgs = function createSwapMsgs(_ref2, sender) {
  ...
  if (isNative) {
    return [new terra_js.MsgExecuteContract(sender, contract_addr, {
      swap: {
        offer_asset: offerAsset,
        max_spread: slippage,
        belief_price: price
      }
    }, [new terra_js.Coin(token, amount)])];
  }

  return [new terra_js.MsgExecuteContract(sender, token, {
    send: {
      amount: amount,
...
var TerraswapContext = /*#__PURE__*/React.createContext({
  pairs: [],
  routes: null,
  tokens: null,
  data: null
...
var TerraswapProvider = function TerraswapProvider(_ref) {
  ...
  var _useTerraWebapp = terra.useTerraWebapp(),
...
function useTerraswap() {
  return React.useContext(TerraswapContext);
}
...
var useSwapSimulate = function useSwapSimulate(_ref) {
  ...
  var _useQuery = reactQuery.useQuery(["simulation", swapRoute, router, token, amount, reverse], function () {
  ...
  return React.useMemo(function () {
    ...
    if (isMultiSimulation(data)) {
      return {
        ...
        spread: "0",
        commission: "0",
        price: terra.num(amount).div(data.amount).toFixed(18)
      };
    }
    ...
    if (isReverseSimulation(data)) {
      return {
        ...
        price: terra.num(data.offer_amount).div(amount).toFixed(18)
      };
    }
    ...
    return {
      ...
      spread: spread,
      commission: commission,
      price: terra.num(amount).div(data.return_amount).toFixed(18)
    };
...
var useSwap = function useSwap(_ref) {
  ...
  var minReceive = React.useMemo(function () {
  ...
  var msgs = React.useMemo(function () {
  ...
  var _useTransaction = terra.useTransaction({
    msgs: msgs,
    onSuccess: onSuccess,
    onError: onError
  }),
      submit = _useTransaction.submit,
      rest = _objectWithoutPropertiesLoose__default["default"](_useTransaction, _excluded);

  return _extends__default["default"]({}, rest, {
    simulated: simulated,
...
var useShareOfPool = function useShareOfPool(_ref) {
  var pool = _ref.pool,
      amount1 = _ref.amount1;
  return React.useMemo(function () {
    if (pool == null || amount1 == null || terra.num(pool.total_share).isEqualTo(0)) {
      return null;
    }

    var token1Amount = pool.assets[0].amount;
    return terra.num(amount1).div(token1Amount).toFixed(2);
  }, [pool, amount1]);
};
...
var useTokensToLp = function useTokensToLp(_ref) {
  ...
  return React.useMemo(function () {
    if (pool == null || amount1 == null || shareOfPool == null || terra.num(amount1).isEqualTo(0)) {
      return null;
    }

    return terra.num(shareOfPool).times(pool.total_share).toFixed();
...
var useLpToTokens = function useLpToTokens(_ref) {
  var pool = _ref.pool,
      amount = _ref.amount;
  return React.useMemo(function () {
    if (pool == null || amount == null || terra.num(amount).isEqualTo(0)) {
      return null;
    }
    ...
    return assets.reduce(function (acc, asset) {
      ...
      return _extends__default["default"]({}, acc, (_extends2 = {}, _extends2[getTokenDenom(asset.info)] = terra.num(amount).times(asset.amount).div(total_share).toFixed(), _extends2));
    }, {});
  }, [pool, amount]);
};
...
var useTokenPriceInUst = function useTokenPriceInUst(token) {
  ...
  return terra.num("1").div(data.price).times(ONE_TOKEN).toFixed();
};
...
var useTotalShareInUst = function useTotalShareInUst(_ref) {
  ...
  var tokenAmounts = React.useMemo(function () {
    var _ref2;
  ...
  return React.useMemo(function () {
    if (pool == null || token1 == null || token2 == null || token1Price == null || token2Price == null || tokenAmounts == null || terra.num(pool.total_share).isEqualTo(0)) {
      return null;
    }

    var totalPrice1 = terra.num(tokenAmounts[token1]).times(token1Price).div(ONE_TOKEN);
    var totalPrice2 = terra.num(tokenAmounts[token2]).times(token2Price).div(ONE_TOKEN);
...
var useTokenInfo = function useTokenInfo() {
  ...
  var getSymbol = React.useCallback(function (token) {
  ...
  var getIcon = React.useCallback(function (token) {
    if (data == null || token == null) {
      return null;
    }
